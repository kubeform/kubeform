...
func NewAwsVpcPeeringConnectionOptionsInformer(client versioned.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredAwsVpcPeeringConnectionOptionsInformer(client, resyncPeriod, indexers, nil)
}

// NewFilteredAwsVpcPeeringConnectionOptionsInformer constructs a new informer for AwsVpcPeeringConnectionOptions type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredAwsVpcPeeringConnectionOptionsInformer(client versioned.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().AwsVpcPeeringConnectionOptionses().List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().AwsVpcPeeringConnectionOptionses().Watch(options)
			},
		},
		&awsv1alpha1.AwsVpcPeeringConnectionOptions{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *awsVpcPeeringConnectionOptionsInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredAwsVpcPeeringConnectionOptionsInformer(client, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *awsVpcPeeringConnectionOptionsInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.AwsVpcPeeringConnectionOptions{}, f.defaultInformer)
}
