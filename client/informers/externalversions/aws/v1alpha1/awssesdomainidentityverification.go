...
func NewAwsSesDomainIdentityVerificationInformer(client versioned.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredAwsSesDomainIdentityVerificationInformer(client, resyncPeriod, indexers, nil)
}

// NewFilteredAwsSesDomainIdentityVerificationInformer constructs a new informer for AwsSesDomainIdentityVerification type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredAwsSesDomainIdentityVerificationInformer(client versioned.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().AwsSesDomainIdentityVerifications().List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().AwsSesDomainIdentityVerifications().Watch(options)
			},
		},
		&awsv1alpha1.AwsSesDomainIdentityVerification{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *awsSesDomainIdentityVerificationInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredAwsSesDomainIdentityVerificationInformer(client, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *awsSesDomainIdentityVerificationInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.AwsSesDomainIdentityVerification{}, f.defaultInformer)
}
