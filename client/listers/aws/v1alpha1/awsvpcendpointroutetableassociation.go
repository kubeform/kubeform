...
type AwsVpcEndpointRouteTableAssociationLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.AwsVpcEndpointRouteTableAssociation, err error)
	// Get retrieves the AwsVpcEndpointRouteTableAssociation from the index for a given name.
	Get(name string) (*v1alpha1.AwsVpcEndpointRouteTableAssociation, error)
...
// List lists all AwsVpcEndpointRouteTableAssociations in the indexer.
func (s *awsVpcEndpointRouteTableAssociationLister) List(selector labels.Selector) (ret []*v1alpha1.AwsVpcEndpointRouteTableAssociation, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.AwsVpcEndpointRouteTableAssociation))
	})
	return ret, err
}

// Get retrieves the AwsVpcEndpointRouteTableAssociation from the index for a given name.
func (s *awsVpcEndpointRouteTableAssociationLister) Get(name string) (*v1alpha1.AwsVpcEndpointRouteTableAssociation, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("awsvpcendpointroutetableassociation"), name)
	}
	return obj.(*v1alpha1.AwsVpcEndpointRouteTableAssociation), nil
}
